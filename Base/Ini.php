...
class Ini {
	...
	public function initGobal($arr)
	{
		if($this->globals===null){
			if(is_array($GLOBALS['CONF']))$this->globals=$GLOBALS['CONF'];
			else $this->globals=array();
		}
		foreach($arr as $k=>$v)$this->globals[$k]=$v;
	}
	public function initCache($basedir,$type=null)
	{
		$this->cacheFunc = $type;
		$this->basedir=$basedir;
		if($type=='yac')$this->cacheSys = new Yac();
	}
	...
	protected function yac_get($item)
	{
		...
		if($v===false){
			...
			if(!empty($conf)) $this->cacheSys->set($item,$conf);
			return $conf;
		}else return $v;
	}
	protected function yac_del($items)
	{
		...
		foreach($items as $key){
			if($this->cacheSys->delete($key))$realDelete++;
		}
		if($realDelete===0)return true;
		else return $realDelete;
	}
	...
	{
		if ($this->globals ===null)$this->globals = $GLOBALS['CONF'];
		if(isset($this->globals[$item]))return $this->globals[$item];
		$path = explode('.', $item);
		switch (sizeof($path)){
